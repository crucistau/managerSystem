<template>
  <div>
    ...
    <div style="padding: 10px 0">
      <el-input style="width: 200px;margin-right: 5px" placeholder="请输入名字" suffix-icon="el-icon-search"
    ...
    >
      ...
      <el-table-column prop="manName" label="姓名" width="260" align="center">
      </el-table-column>
      <el-table-column prop="manId" label="Id" width="240" align="center">
      </el-table-column>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="姓名" prop="name">
              <el-input v-model="formA.manName"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="管理员Id" prop="name">
              <el-input v-model="formA.manId"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="邮箱" prop="faculty">
...
export default {
  ...
  methods: {
    ...
    //分页+条件查询
    getAll() {
      this.axios.get("/manage/" + this.pagination.currentPage + "/" + this.pagination.pageSize, {
        params: {
          manName: this.search.name,
    ...
    //增添学生信息
    handleAdd() {
      this.axios.post("/manage", this.formA).then(resp => {
        if (resp.code === '200') {
          //增添成功 关闭弹窗
          this.dialogFormVisibleEdit = false
          this.dialogFormVisible = false
    ...
    //编辑信息
    handleEdi() {
      this.axios.put("/manage", this.form).then(resp => {
        if (resp.code === '200') {
          //增添成功 关闭弹窗
          this.dialogFormVisibleEdit = false
          this.dialogFormVisible = false
    ...
    handleUpdate(row) {
      //将数据表示在编辑窗口上
      axios.get("/manage/" + row.id).then((resp) => {
        console.log(resp)
    ...
    handleDelete(row) {
      this.$confirm("此操作将永久删除当前信息，是否继续？", "提示", {type: "info"}).then(() => {
        axios.delete("/manage/" + row.id).then(resp => {
          if (resp.code === '200') {
    ...
    del() {
      ...
      console.log(ids);
      axios.post("/manage/del/batch", ids).then(resp => {
        if (resp.code === '200') {
          this.$message.success("删除成功");
        } else {
          this.$message.error(resp.msg);
